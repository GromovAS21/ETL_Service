...
def load_jobs() -> list[Path]:
    ...
    return [
        file for file in JOBS_DIR.iterdir() if file.is_file() and file.suffix == ".yaml"
    ]
...
def read_job(job_path: Path) -> JobSchema:
    ...
    try:
        with open(job_path, encoding="utf-8") as f:
            # raw = f.read()
            # logger.info(raw)
            return JobSchema(**yaml.safe_load(f))
...
def extract_data(
    main_cursor: Cursor, query: str, hash_date: bytes | None, batch_size: int = 50
...
def create_delete_query(
    key_columns: List[str], values_in_main_db: List[tuple], name_table: str
) -> Composed:
    ...
    if values_in_main_db:
        if len(key_columns) > 1:
            ...
            query = sql.SQL(
                "DELETE FROM {name_table} WHERE ({name_columns}) NOT IN ({values})"
            ).format(
                name_table=sql.Identifier(name_table),
            ...
            query = sql.SQL(
                "DELETE FROM {name_table} WHERE {name_column} NOT IN ({values})"
            ).format(
                name_table=sql.Identifier(name_table),
...
def process_job(job_path: Path) -> None:
    ...
    try:
        with (
            get_main_db_cursor() as main_cursor,
            ...
            logger.info(
                "Вставлено/обновлено %d записей в таблицу %s",
                len(data_list),
                job.target_table,
            )
            ...
            delete_query = create_delete_query(
                job.key_columns, values_in_main_db, job.target_table
            )
            analytic_cursor.execute(delete_query)
            deleted_count = analytic_cursor.rowcount
            logger.info(
                "Удалено %d записей из таблицы %s", deleted_count, job.target_table
            )
